...
public class AlumnoController {
	...
	/* Solución Ayrton Barriga
	@PutMapping
	@ResponseBody
	public ResponseEntity<HashMap<String, Object>> actualizarAlumno(@RequestBody Alumno obj) {
		HashMap<String, Object> salida = new HashMap<String, Object>();
		try {
			Optional<Alumno> lstAlumnos = service.listaAlumnoPorId(obj.getIdAlumno());
			List<Alumno> lstAlumnos2 = service.listaAlumnoPorDniDiferenteDelMismo(obj.getDni(), obj.getIdAlumno());
			if (lstAlumnos.isPresent() && (CollectionUtils.isEmpty(lstAlumnos2))) {				
				//obj.setIdAlumno(0);
				Alumno objSalida = service.insertaActualizaAlumno(obj);
				if (objSalida == null) {
				...
				salida.put("mensaje", "El DNI no existe");
			}
		} catch (Exception e) {
			e.printStackTrace();
			salida.put("mensaje", "Error en el actualizacion " + e.getMessage());
		}
		return ResponseEntity.ok(salida);
	}*/
	@PutMapping
